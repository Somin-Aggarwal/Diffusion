...
weights_path = "classifier_weights/classifier_best.pt"
weights = torch.load(weights_path, map_location=device)
from model import UNET  # make sure model.py is in path
classifier = MNISTClassifier(image_channels=1,
...
for time_step in tqdm(reversed(range(1, steps + 1))):
    for m in range(M):
        ...
        uncond_score = - ( predicted_noise / sqrt_one_minus_alphas_cumprod[t_idx] )
        cond_score = get_classifier_score(new_image,t,target_number,classifier,guidance_scale=1.0)
        pred_score = uncond_score + 1.5*cond_score
