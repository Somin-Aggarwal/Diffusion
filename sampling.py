...
class SamplingClass():
    def __init__(self,model,batch_size,schedule,steps,img_ch,n_classes,device):
        self.model = model
    ...
    def Ancestral2(self, labels:list, weight:float):
    ...
    def DDIM(self, labels, time_steps):
    ...
    def DDIM_Interpolation(self, time_steps):
        ...
        labels = torch.full(size=(self.batch_size,),fill_value=1,dtype=torch.long,device=self.device)
