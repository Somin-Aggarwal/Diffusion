...
weights_path = "mnist_clsfree/best_model.pt"
...
new_image = torch.randn(size=(batch_size, mcfg['img_ch'], image_size, image_size), device=device)
images = [[] for _ in range(batch_size)]  # each sublist holds frames for one sample

M = 0 
labels = torch.randint(low=0,high=10,size=(batch_size,)).to(device)
print(labels)
...
with torch.no_grad():
    for time_step in tqdm(reversed(range(1, steps + 1)),total=steps):
        ...
        weight = 0.9
        pred_score = weight * pred_score_cond + (1-weight) * pred_score_uncond 
        ...
        if time_step > 1:
            for m in range(M):
                ...
                predicted_noise = model(new_image, t, labels)
